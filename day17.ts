...
const part = async (arr: string[]) => {
  ...
  // draw(output);
  return buildMap(output);
};

const part1 = async (arr: string[]) => {
  return intersectionMulter(await part(arr));
};

const part2 = async (arr: string[]) => {
  const graph = await part(arr);
  const findStart = (arr: string[][]): number[] => {
    for (let y = 0; y < arr.length; y++) {
      const row = arr[y].join("");
      if (row.indexOf("^") > -1) {
        return [y, row.indexOf("^"), 0];
      }
    }
    return [-1, -1, -1];
  };
  let pos = findStart(graph);
  // p({ pos });
  const dr = [-1, 0, 1, 0];
  const dc = [0, 1, 0, -1];
  const nextF = (graph: string[][]) =>
    (pos: number[]) => {
      const row = pos[0] + dr[pos[2]];
      const col = pos[1] + dc[pos[2]];
      // p({ row, col });
      if (
        row < 0 || col < 0 || row >= graph.length || col >= graph[0].length
      ) {
        return [".", ...pos];
      }
      // p({ g: graph[row][col] });
      return [graph[row][col], row, col, pos[2]];
    };
  const next = nextF(graph);
  const right = (pos: number[]) => {
    const ans = pos.slice(0, 2);
    ans.push((pos[2] + 1) % 4);
    return ans;
  };
  const left = (pos: number[]) => {
    const ans = pos.slice(0, 2);
    ans.push((pos[2] + 3) % 4);
    return ans;
  };
  ...
  while (true) {
    ...
    if (next(pos)[0] === "#") {
      count++;
      pos = next(pos).slice(1).map(Number);
    } else if (next(right(pos))[0] === "#") {
      if (count > 0) route.push(String(count));
      count = 0;
      route.push("R");
      pos = right(pos);
    } else if (next(left(pos))[0] === "#") {
      if (count > 0) route.push(String(count));
      count = 0;
      route.push("L");
      pos = left(pos);
...
const part22 = async (arr: string[]) => {
  ...
  const A = `R,12,L,8,R,6`;
  const B = `R,12,L,6,R,6,R,8,R,6`;
  const C = `L,8,R,8,R,6,R,12`;
  p(route.replaceAll(A, "A").replaceAll(B, "B").replaceAll(C, "C"));
  p(route.replaceAll(A, "A").replaceAll(B, "B").replaceAll(C, "C").length);
  p(A.length);
  p(B.length);
  p(C.length);
  const ans = route.replaceAll(A, "A").replaceAll(B, "B").replaceAll(C, "C")
    .split("")
    .map((s) => s.codePointAt(0));
  ans.push(10);
  ans.push(...A.split("").map((s) => s.codePointAt(0)));
  ans.push(10);
  ans.push(...B.split("").map((s) => s.codePointAt(0)));
  ans.push(10);
  ans.push(...C.split("").map((s) => s.codePointAt(0)));
  ans.push(10);
  ans.push("n".codePointAt(0));
  ans.push(10);
  p({ ans });
  p(ans.length);
  let stepper: Step = {
...
console.timeLog("p");
await part2(raw.split(","));
await part22(raw.replace("1", "2").split(","));
console.timeEnd("p");
