...
const part1 = async (arr: string[]) => {
  const moves1 = await part(arr, 0);
  return new Set(moves1.map((e) => {
    const pt = { x: e.x, y: e.y };
    return JSON.stringify(pt);
  })).size;
};
const part2 = async (arr: string[]) => {
  const moves = await part(arr, 1);
  const x = moves.reduce((acc, val) => val.x < acc ? val.x : acc, 0);
  const y = moves.reduce((acc, val) => val.y < acc ? val.y : acc, 0);
  const graph: string[][] = [];
  moves.forEach((e) => {
    const dx = e.x - x;
    const dy = e.y - y;
    if (graph[dx] === undefined) graph[dx] = [];
    graph[dx][dy] = e.color === 1 ? "#" : "_";
  });
  for (const row of graph) {
    p(row.join(""));
  }
};
p(`p1: unique steps: ${await part1(input.split(","))}`);
await part2(input.split(","));
