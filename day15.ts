import { delay as dp } from "https://deno.land/std/async/mod.ts";
const input =
...
const allDir = (pt: Point) => {
  return [
    { x: pt.x, y: pt.y - 1, i: 3, steps: pt.steps + 1 },
    { x: pt.x - 1, y: pt.y, i: 2, steps: pt.steps + 1 },
    { x: pt.x, y: pt.y + 1, i: 4, steps: pt.steps + 1 },
    { x: pt.x + 1, y: pt.y, i: 1, steps: pt.steps + 1 },
  ];
};

const isNotVisited = (visited: Point[]) =>
  (curr: Point) => !visited.some((v) => v.x === curr.x && v.y === curr.y);

const updateVisited = (curr: Point, visited: Point[]) => {
  visited.push(curr);
};
const nextChoices = (curr: Point, visited: Point[]) => {
  const f = isNotVisited(visited);
  const res = allDir(curr).filter(f);
  return res;
  /* 
  const g = isVisited(visited);
  res.unshift(...allDir(curr).filter(g).filter(removeBlocks));
  return res.map((pt) => {
    const found = visited.find((v) => v.x === pt.x && v.y === pt.y);
    if (found && found.steps < pt.steps) pt.steps = found.steps;
    return pt;
  });
  */
};

const draw = (visited: Point[], pt: Point) => {
  const arr: number[][] = [];
  const dx = visited.map((p) => p.x).reduce((a, b) => a > b ? b : a, 0);
  const dy = visited.map((p) => p.y).reduce((a, b) => a > b ? b : a, 0);
  visited.map((p) => {
    return {
      x: p.x - dx,
      y: p.y - dy,
    ...
    if (!arr[p.y]) arr[p.y] = [];
    if (p.o !== undefined) arr[p.y][p.x] = p.o;
  });
  arr[pt.y - dy][pt.x - dx] = 8;
  for (const row of arr) {
    p(row.join("").replaceAll("0", "#").replaceAll("1", "."));
  }
};
...
const part1 = async (arr: string[]) => {
  ...
  const start = { x: 0, y: 0, steps: 0, o: 1, i: 0 };
  const visited: Point[] = [start];
  const queue: Point[] = allDir(start);
  let check = queue.pop();
  if (check) stepper.input = check.i;
  while (!stepper.halt && check ) {
    stepper = await step(stepper);
    if (stepper.output !== undefined) {
      const output = Number(stepper.output);
      check.o = output;
      p({ check, l: queue.length });
      updateVisited(check, visited);
      // draw(visited, check);
      if (output === 1) { // not hit the wall, move check to curr and generate new options
        const nextC = nextChoices(check, visited);
        p({ nextC });
        //   await dp(500);
        queue.push(...nextC);
      }
